...
from data_utils import load_MNIST_data, load_Fashion_MNIST_data, \
    load_CIFAR_data, load_CIFAR_100_data, load_EMNIST_data, generate_bal_private_data, generate_partial_data
...
if __name__ == "__main__":
    ...
    if model_saved_dir is None:
        ...
        pre_train_result = train_models(parties,
                                        public_dataset["X"], public_dataset["y"],
                                        public_test_dataset["X"], public_test_dataset["y"],
    ...
    with open(os.path.join(save_dir_path, 'col_performance_gan_new_'+str(Public_dataset)+'_'+str(N_rounds)+'_'+str(N_alignment)+'.pkl'), 'wb') as f:
